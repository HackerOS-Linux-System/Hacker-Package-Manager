...
fn get_hpm_apt_path() -> Result<String> {
    std::env::var("HOME")
        .map(|home| format!("{}/.hackeros/hpm/hpm-apt", home))
...
fn install_package(package: &str, log_file: &mut File) -> Result<()> {
    ...
    let hpm_apt_path = get_hpm_apt_path()?;
    // Get URIs to download
    let uris_output = get_command_output(&hpm_apt_path, &["--print-uris", "-y", "install", package], log_file, false)?;
...
fn clean_packages(log_file: &mut File) -> Result<()> {
    let hpm_apt_path = get_hpm_apt_path()?;
    run_command(&hpm_apt_path, &["autoclean"], log_file, "Running autoclean", true)?;
    run_command(&hpm_apt_path, &["autoremove"], log_file, "Running autoremove", true)?;
...
fn update_packages(log_file: &mut File) -> Result<()> {
    ...
    let sim_output = get_command_output(&hpm_apt_path, &["-s", "-y", "upgrade"], log_file, false)?;
    if !sim_output.contains("Inst ") {
        println!("{}", "All packages are up to date.".green().bold());
        return Ok(());
    }
    // Get URIs for upgrade
    let uris_output = get_command_output(&hpm_apt_path, &["--print-uris", "-y", "upgrade"], log_file, false)?;
    let downloads = parse_print_uris_output(&uris_output);
    if downloads.is_empty() {
...
fn refresh_packages(log_file: &mut File) -> Result<()> {
    let hpm_apt_path = get_hpm_apt_path()?;
    run_command(&hpm_apt_path, &["update"], log_file, "Refreshing package lists", true)?;
